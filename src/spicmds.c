...
enum {
    SF_HAVE_PIN = 1, SF_SOFTWARE = 2, SF_HARDWARE = 4, SF_INVERT_PIN = 8
};
...
command_config_spi(uint32_t *args)
{
    ...
    int_fast8_t invert_pin = args[2];
    spi->flags |= SF_HAVE_PIN;
    if(invert_pin) spi->flags |= SF_INVERT_PIN;
    int_fast8_t pin_inactive_value = invert_pin ? 0 : 1;
                ...
                , uint8_t data_len, uint8_t *data)
{
    ...
    int_fast8_t pin_active_value = (spi->flags & SF_INVERT_PIN) ? 1 : 0;
...
spidev_shutdown(void)
{
    ...
    foreach_oid(oid, spi, command_config_spi) {
        int_fast8_t pin_inactive_value = (spi->flags & SF_INVERT_PIN) ? 0 : 1;
        if (spi->flags & SF_HAVE_PIN)
