...
class DRV8711:
  def __init__(self, config):
    ...
    self.regs['MODE'] = config.getchoice('microsteps', MODE_OPTIONS, default = 32)

    self.regs['ISGAIN'] = config.getchoice('gain', ISGAIN_OPTIONS, default = 20)
    gain = [k for k, v in ISGAIN_OPTIONS.items() if v == self.regs['ISGAIN']][0]
    ...
    self.regs['DTIME'] = config.getchoice('DTIME', DTIME_OPTIONS, default=850)
    self.regs['TOFF'] = \
      int(config.getfloat('TOFF_USECS', default=76, minval=0.5, maxval=128.)*2) - 1
    self.regs['TBLANK'] = \
      int(config.getfloat('TBLANK_USECS', default=4.3, minval=1., maxval=5.12)/0.020)
    self.regs['ABT'] = config.getboolean('ABT', True)
    self.regs['TDECAY'] = \
      int(config.getfloat('TDECAY_USECS',minval=0., maxval=127.5, default=24)*2)
    self.regs['DECMOD'] = config.getint('DECMOD',minval=0, maxval=5, default=4)

    self.regs['OCPTH'] = config.getchoice('OCPTH', OCPTH_OPTIONS, default=250)
    self.regs['OCPDEG'] = config.getchoice('OCPDEG', OCPDEG_OPTIONS, default=1)
    self.regs['TDRIVEN'] = config.getchoice('TDRIVEN', TDRIVEN_OPTIONS, default=2000)
    self.regs['TDRIVEP'] = config.getchoice('TDRIVEP', TDRIVEP_OPTIONS, default=2000)
    self.regs['IDRIVEN'] = config.getchoice('IDRIVEN', IDRIVEN_OPTIONS, default=100)
    self.regs['IDRIVEP'] = config.getchoice('IDRIVEP', IDRIVEP_OPTIONS, default=50)


  def handle_connect(self):
    # Note: Some registers are kept at fixed values, since changing them does not make
    # sense for our use cases. Also stall detection is not supported (yet).
    ENBL = 1
