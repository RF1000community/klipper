...
class MCU_ADS1100:
    ...
    def _handle_ready(self):
        # configure adc on next timer event
        self._config_done = False

        # setup readout timer
        self._sample_timer = self._reactor.register_timer(self._handle_timer,
            self._reactor.NOW+self._sample_time)
    ...
    def _handle_timer(self, eventtime):
        ...
        if self._last_value < self._minval or self._last_value > self._maxval :
          ...
          if self._error_count >= self._range_check_count :
            self._printer.invoke_shutdown(f"ADC (ADS1100) out of range, {self._last_value} is not in [{self._minval}, {self._maxval}]")
        else :
          self._error_count = 0
