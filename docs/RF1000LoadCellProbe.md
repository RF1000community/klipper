...
[reference of the config section](Config_Reference.md#rf1000_load_cell_probe), and the
[reference of the g-code commands](G-Codes.md#rf1000_load_cell_probe).
...
`printer["rf1000_load_cell_probe"].last_z_result`.
...
config sections trough `printer["rf1000_load_cell_probe"].last_force`. This value will
...
1. Make sure the load cell reading is sane. Ideally, place the force readout
   (`printer["rf1000_load_cell_probe"].last_force`) on the display, so it can be
...
gcode:
    SET_GCODE_OFFSET Z={printer["rf1000_load_cell_probe"].last_z_result}
